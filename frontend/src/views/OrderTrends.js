        ...
        <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'white' }}>
            <div style={{ width: '150px', backgroundColor: '#D3D3D3', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <button
                    ...
                    style={{ marginTop: 'auto', padding: '10px', backgroundColor: 'black', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>
                    Back
                </button>
            </div>
            <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                    <button onClick={fetchXReport} disabled={loading} style={{ padding: '10px 15px', backgroundColor: activeReport === 'x' ? 'green' : '#D3D3D3', color: activeReport === 'x' ? 'white' : 'black', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>X-Report</button>
                    <button onClick={fetchZReport} disabled={loading} style={{ padding: '10px 15px', backgroundColor: activeReport === 'z' ? 'green' : '#D3D3D3', color: activeReport === 'z' ? 'white' : 'black', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Z-Report</button>
                    <button onClick={() => setScreen("graph")} style={{ padding: '10px 15px', backgroundColor: 'blue', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Product Usage Chart</button>
                    <button onClick={resetReports} style={{ padding: '10px 15px', backgroundColor: 'red', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', marginLeft: 'auto' }}>Clear</button>
                </div>
                ...
                {activeReport === 'x' && (
                    <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', overflow: 'hidden' }}>
                        <h2 style={{ padding: '15px', backgroundColor: '#333333', color: 'white', margin: 0 }}>X-Report (Updated: {formatTime(currentTime)})</h2>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                ...
                                <tbody>
                                    {xReport.length > 0 ? (
                                        ...
                                        <tr>
                                            <td colSpan="7" style={{ textAlign: 'center', padding: '20px' }}>No order data available for today</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                ...
                {activeReport === 'z' && zReport && (
                    <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '20px', backgroundColor: '#D3D3D3' }}>
                            ...
                            <div style={zReportItemStyle}><span style={zReportLabelStyle}>Total Employees:</span><span style={zReportValueStyle}>{zReport.total_employees}</span></div>
                            <div style={zReportItemStyle}><span style={zReportLabelStyle}>Total Items Sold:</span><span style={zReportValueStyle}>{zReport.total_items}</span></div>
                            <div style={zReportItemStyle}><span style={zReportLabelStyle}>Apple Pay Transactions:</span><span style={zReportValueStyle}>{zReport.apple_pay_count}</span></div>
                            <div style={zReportItemStyle}><span style={zReportLabelStyle}>Credit/Debit Transactions:</span><span style={zReportValueStyle}>{zReport.card_count}</span></div>
                        </div>
                    </div>
                )}
            </div>
